...
export class OpenAIProvider implements TTSProvider {
    ...
    async speak(text: string, apiKey: string, voiceId: string = 'alloy'): Promise<ArrayBuffer> {
        try {
            ...
            if (error.message?.includes('character') || error.message?.includes('limit') || error.message?.includes('maximum')) {
                throw new Error(
                    `OpenAI Error: Text is too long. OpenAI has a limit of 4,096 characters. ` +
                    `Your text is ${text.length} characters. Please reduce by ${text.length - 4096} characters.`
                );
            }
